...
import { Building2, Users, Lightbulb, FileText, Scale as ScaleBalance, Home, Gem, TreeDeciduous, Droplets, Fish, MapPin } from 'lucide-react';
...
const PracticeAreaCard: React.FC<PracticeAreaCardProps> = ({ practiceArea }) => {
  const getIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'Gem':
        return <Gem size={36} className="text-primary mb-4" />;
      case 'TreeDeciduous':
        return <TreeDeciduous size={36} className="text-primary mb-4" />;
      case 'Droplets':
        return <Droplets size={36} className="text-primary mb-4" />;
      case 'Fish':
        return <Fish size={36} className="text-primary mb-4" />;
      case 'MapPin':
        return <MapPin size={36} className="text-primary mb-4" />;
      default:
        return <Building2 size={36} className="text-primary mb-4" />;
    }
  };
